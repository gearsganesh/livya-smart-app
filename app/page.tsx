...
const patientScreens: Screen[] = [
  ...
  { id: 'schedule', label: 'Schedule', icon: CalendarDays }, { id: 'ai', label: 'LIVYA AI', icon: Sparkles }, { id: 'profile', label: 'Profile', icon: UserRound },
];
...
export default function Home() {
  ...
  useEffect(() => {
    let mounted = true;
    supabase.auth.getSession().then(({ data }) => { if (mounted) loadUser(data.session?.user ?? null); });
  ...
  async function loadUser(user: { id: string; email?: string | null } | null) {
    ...
    setSession({ userId: user.id, email: user.email ?? null }); setProfile(p); setPatient(pt); setIsStaff(Boolean(staff));
    setMode(staff ? 'admin' : 'patient'); setScreen(staff ? 'dashboard' : 'home'); setLoading(false);
  }
  ...
  return <main className="app-shell">
    <aside className={`rail ${drawer ? 'open' : ''}`}><div className="brand"><div className="brand-mark">L</div><span>LIVYA AI</span></div><div className="mode-switch"><button className={mode === 'patient' ? 'active' : ''} onClick={() => switchMode('patient')}>Patient</button><button className={`${mode === 'admin' ? 'active' : ''} ${!isStaff ? 'disabled' : ''}`} onClick={() => switchMode('admin')}>Admin</button></div><nav>{screens.map(({ id, label, icon: Icon }) => <button key={id} className={screen === id ? 'nav-item active' : 'nav-item'} onClick={() => { setScreen(id); setDrawer(false); }}><Icon size={17} /><span>{label}</span></button>)}</nav><button className="signout" onClick={() => supabase.auth.signOut()}><LogOut size={16} /> Sign out</button></aside>
    <section className="workspace"><header className="topbar"><button className="mobile-menu" onClick={() => setDrawer(!drawer)} aria-label="Menu"><Menu size={21} /></button><div><div className="eyebrow">{mode === 'patient' ? 'PATIENT APP' : 'ADMIN CONSOLE'}</div><h1>{active.label}</h1></div><div className="top-actions"><button className="circle"><Bell size={18} /></button><button className="circle"><MessageCircle size={18} /></button><div className="avatar">{initials(profile?.full_name || profile?.email || 'LIVYA')}</div></div></header>{mode === 'patient' ? <PatientView patient={patient} profile={profile} screen={screen} /> : <AdminView screen={screen} />}</section>
...
function PatientView({ patient, profile, screen }: { patient: Patient | null; profile: UserProfile | null; screen: string }) {
  ...
  const copy: Record<string, { title: string; text: string }> = { medicines: { title: 'Medicines', text: 'Medication schedule, adherence, stock, refill and prescription-linked ordering.' }, records: { title: 'Health records', text: 'Upload reports, review AI extraction and compare health markers over time.' }, nutrition: { title: 'My plan', text: 'Programme targets, diet chart, recipes, coach notes and progress.' }, wellness: { title: 'Wellness', text: 'Therapies, sessions, therapists, bookings and plan consumption.' }, store: { title: 'Store', text: 'Prescribed medicines, devices, supplements, cart and checkout.' }, labs: { title: 'Lab tests', text: 'Advised tests, partner labs, home collection and slot booking.' }, schedule: { title: 'Schedule', text: 'Consultations, therapy, treatment and lab appointments in one calendar.' }, ai: { title: 'LIVYA AI', text: 'A patient-aware assistant that can use authorised records, medicines and programme context.' }, profile: { title: 'Profile', text: 'Personal details, family members, membership and connected health settings.' } };
